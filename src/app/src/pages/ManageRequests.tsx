...
type RequestProps = 'Requests' | 'Friends' | 'Following' | 'Followers';
...
export default function ManageRequests(): JSX.Element {
  ...
  const buttons: Array<RequestProps> = ['Requests', 'Friends', 'Following', 'Followers'];

  const handleonClick = (type: RequestProps) => {
    switch (type) {
      case 'Requests':
  ...
  const returnLength = (type: RequestProps) => {
    switch (type) {
      case 'Requests':
  ...
  const returnKey = (type: RequestProps) => {
    switch (type) {
      case 'Requests':
        return 'requests';
      case 'Friends':
  ...
  const lists = [
    followRequests.map((user) => <ManageRequestsRequests user={user} key={user.id} />),
    friends.map((friend) => <ManageRequestsFriends user={friend} key={friend.id} />),
    following.map((user) => <ManageRequestsFollowing user={user} key={user.id} />),
    followers.map((user) => <ManageRequestsFollowers user={user} key={user.id} />),
  ];
        ...
        <Box style={{ display: 'flex', height: '95%' }} sx={{ bgcolor: '#fff' }}>
          ...
          >
            <ButtonGroup
              ...
              fullWidth={true}
            >
              {buttons.map((item: RequestProps) => (
                <Button onClick={() => handleonClick(item)} key={returnKey(item)} sx={buttonSx}>
                  {item}
                  <Badge badgeContent={returnLength(item)} color="secondary" sx={badgeSx} />
                </Button>
              ))}
            </ButtonGroup>
          </Box>
